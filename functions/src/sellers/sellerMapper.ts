...
function pickString(r: CrmSellerRecord, keys: string[]): string | undefined {
  for (const k of keys) {
    const v = r[k];
...
function pickBool(r: CrmSellerRecord, keys: string[]): boolean | undefined {
  for (const k of keys) {
    const v = r[k];
    ...
    if (typeof v === "string") {
      ...
      if (["false", "0", "no", "n", "nao", "não", "inativo"].includes(s)) return false;
    }
  }
  return undefined;
}

export function getExternalIdFromCrmSeller(r: CrmSellerRecord): string {
  const id =
    pickString(r, ["id", "externalId", "sellerId", "sellerExternalId", "uuid"]) ??
...
export function mapCrmSellerToDirectoryDoc(r: CrmSellerRecord, nowIso: string): SellerDirectoryDoc {
  return {
...
export function safeFirestoreDocId(externalId: string): string {
