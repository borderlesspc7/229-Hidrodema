...
const EqualizadorServicos = () => {
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      const v = validateEqualizadorFormData(formData as Record<string, unknown>);
      if (!v.ok) {
  ...
  // Exportar MDS para impressão (Salvar como PDF)
  const handleExportPDF = async (service: ServiceMDS) => {
    try {
      setLoading(true);
      const comments = await getCommentsByMDSId(service.id);
      const data = service.formData ?? {};
      const mdsNumber = service.mdsNumber || service.number;
      const client = (data.q1 as string) || service.client || "-";
      const workLocation = (data.q2 as string) || service.project || "-";
      const visitDate = (data.q3 as string) || "-";
      ...
      console.error(e);
      alert("Erro ao exportar MDS para impressão/PDF.");
    } finally {
      setLoading(false);
    }
  };
