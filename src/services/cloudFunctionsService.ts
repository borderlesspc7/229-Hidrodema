...
export type ExportServiceRequestsCsvInput = {
  status?: string;
  limit?: number;
};

export type ExportServiceRequestsCsvResult = {
  ok: boolean;
  count: number;
  storagePath: string;
  downloadUrl: string;
...
export async function exportServiceRequestsCsv(
  input: ExportServiceRequestsCsvInput = {}
): Promise<ExportServiceRequestsCsvResult> {
  const fn = httpsCallable<
    ExportServiceRequestsCsvInput,
    ExportServiceRequestsCsvResult
  >(functions, "exportServiceRequestsCsv");
...
export async function syncSellerDirectoryExternal(): Promise<SyncSellerDirectoryResult> {
  const fn = httpsCallable<void, SyncSellerDirectoryResult>(
    functions,
    "syncSellerDirectoryExternal"
  );
  const res = await fn();
  return res.data;
}
